...
age_dist_census = pd.read_csv('../../data/external/tabula-ETAT_STRUCTURE_POPULATION.csv')
...
by_region_census_total = age_dist_census_adults.groupby(['Region']).apply(census_sum_total)
...
by_region_voters_total = voters_data.groupby('region').apply(voters_age_distrib).reset_index()
...
g = sns.FacetGrid(data, col="region", col_wrap=2 , size=5 , aspect = .8 , hue="source" , palette ="Set1" , ylim=(-0.001,0.08)).map(plt.scatter ,  'age_num' ,  'percentage' , **kws).set(xlabel='Age', ylabel='Age Distribution').add_legend()
g.savefig('../../reports/figures/age_structure_comparison.png')
